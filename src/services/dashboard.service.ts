...
import { dyCheckIns, dyDrops, dyDropTypes, dySleep, dyTherapySessions } from "../db";
import { isoDateAfterDays, shortDayLabel } from "../lib/dates";
import { getSpearmanCorrelation } from "../lib/stats";
import { buildLastDayKeys, dayKeyToUtcStart, getDayKey } from "../lib/utils";
import type { TriggerType } from "../lib/domain-types";

export const MIN_CORRELATION_SAMPLES = 14;

type SummaryCheckInRow = {
  logged_at: string;
  eyelid_pain: number;
  temple_pain: number;
...
type CorrelationCheckInRow = Omit<SummaryCheckInRow, "trigger_type">;
type SleepRow = { day_key: string; sleep_hours: number };
type TherapyRow = { logged_at: string };

export function correlationInsight(spearman: number | null, samples: number): string {
...
export async function getDashboardSummary(db: DrizzleDb, userId: string, timezone: string) {
  ...
  return buildDashboardSummary(timezone, buildLastDayKeys(timezone, 30), checkInsRows, dropsRows);
}

export function buildDashboardSummary(
  timezone: string,
  last30DayKeys: string[],
  checkInsRows: SummaryCheckInRow[],
  dropsRows: SummaryDropRow[],
) {
  ...
  const trendPoints = last30DayKeys.map((dayKey) => {
    ...
    if (!b || b.count === 0) return { dayKey, label: shortDayLabel(dayKey), eyelidPain: null, templePain: null, masseterPain: null, cervicalPain: null, orbitalPain: null };
    return {
      dayKey,
  ...
  const dropsPoints = last30DayKeys.map((dayKey) => {
    ...
    return { dayKey, label: shortDayLabel(dayKey), quantities };
  });

  const dropsByWeekday = weekdayLabels.map((label, i) => {
...
export async function getDashboardCorrelations(db: DrizzleDb, userId: string, timezone: string) {
  const thresholdDate = new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString();
  const thresholdDayKey = getDayKey(thresholdDate, timezone);
  const utcThreshold = dayKeyToUtcStart(thresholdDayKey, timezone);
  ...
  return buildDashboardCorrelations(timezone, checkInsRows, sleepRows, therapyRows);
}

export function buildDashboardCorrelations(
  timezone: string,
  checkInsRows: CorrelationCheckInRow[],
  sleepRows: SleepRow[],
  ...
  if (therapyRows.length >= 3) {
    ...
    for (const dk of therapyDaySet) {
      const pb = painByDay.get(isoDateAfterDays(dk, 1));
      if (pb) afterTherapyPains.push(pb.sum / pb.count);
    }

    const afterSet = new Set([...therapyDaySet].map((dk) => isoDateAfterDays(dk, 1)));
