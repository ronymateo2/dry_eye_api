...
import { dyUsers, dyCheckIns, dySleep, dyDrops, dyTriggers, dyMedications } from "../db";
import { getDayKey } from "../lib/utils";
import { getSpearmanCorrelation } from "../lib/stats";
import type { TriggerType } from "../lib/domain-types";

export const MIN_REPORT_RECORDS = 14;
...
type ReportSleepRow = { day_key: string; sleep_hours: number; sleep_quality: string };
type ReportDropRow = { logged_at: string; quantity: number };
type ReportTriggerRow = { logged_at: string; trigger_type: string };
type ReportMedRow = {
  name: string;
  dosage: string | null;
...
export function correlationLabel(s: number | null): string {
  if (s === null) return "—";
...
export async function getReport(db: DrizzleDb, userId: string, timezone: string) {
...
export function buildReport(
  ...
  checkInsRows: ReportCheckInRow[],
  sleepRows: ReportSleepRow[],
  dropsRows: ReportDropRow[],
  triggersRows: ReportTriggerRow[],
  medsRows: ReportMedRow[],
) {
  const checkInsCount = checkInsRows.length;
  const hasEnoughData = checkInsCount >= MIN_REPORT_RECORDS;
  ...
  const spearmanRaw = correlationPoints.length >= MIN_REPORT_RECORDS
