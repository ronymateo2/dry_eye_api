...
type ObsRow = { id: string; logged_at: string; intensity: number | null; duration_minutes: number | null; notes: string | null; title: string; obs_eye: string };
...
export async function getInitialHistory(db: DrizzleDb, userId: string, timezone: string) {
...
export async function getMoreHistory(
  db: DrizzleDb,
  userId: string,
  timezone: string,
  beforeDayKey: string,
  ...
  return { ...data, groups: data.groups.slice(0, limitDays), hasMore };
}
...
export function buildGroups(
  ...
  const sleepByDay = new Map<string, HistoryEntry>();
